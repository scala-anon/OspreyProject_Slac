...
std::vector<DecodedPVSeries> DataDecoder::queryAndDecodeByPattern(
    ...
    // Query and decode the discovered PVs using unary method
    return queryAndDecode(pv_names, start_time, end_time, use_serialized);
}

// NEW: Streaming implementation for pattern queries
std::vector<DecodedPVSeries> DataDecoder::queryAndDecodeByPatternStreaming(
    const std::string& pattern,
    uint64_t start_time,
    uint64_t end_time,
    bool use_serialized) {
    ...
    return queryAndDecodeStreaming(pv_names, start_time, end_time, use_serialized);
}

// NEW: Streaming implementation for large datasets
std::vector<DecodedPVSeries> DataDecoder::queryAndDecodeStreaming(
    const std::vector<std::string>& pv_names,
    uint64_t start_time,
    uint64_t end_time,
    bool use_serialized) {

    std::cout << "Using streaming query for " << pv_names.size() << " PVs from "
              << TimeUtils::formatUnixTime(start_time) << " to "
              << TimeUtils::formatUnixTime(end_time) << std::endl;
    ...
    try {
        ...
        std::vector<QueryDataResponse> responses = query_client_->queryDataStream(request);
        
        std::cout << "Received " << responses.size() << " streaming responses" << std::endl;
        
        // Process all streaming responses
        return processStreamingResponses(responses);

    } catch (const std::exception& e) {
        std::cerr << "Streaming query failed: " << e.what() << std::endl;
        return {};
    }
}

// NEW: Process streaming responses
std::vector<DecodedPVSeries> DataDecoder::processStreamingResponses(
    const std::vector<QueryDataResponse>& responses) const {
    
    std::vector<DecodedPVSeries> all_decoded_series;
    
    for (size_t i = 0; i < responses.size(); ++i) {
        std::cout << "Processing streaming response " << (i + 1) << "/" << responses.size() << std::endl;
        
        // Decode each response using existing logic
        auto decoded_series = decodeQueryResponse(responses[i]);
        
        // Accumulate results
        all_decoded_series.insert(all_decoded_series.end(), 
                                 decoded_series.begin(), 
                                 decoded_series.end());
    }
    
    std::cout << "Successfully processed all streaming responses, total PV series: " 
              << all_decoded_series.size() << std::endl;
    
    return all_decoded_series;
}
...
// NEW: Expose queryPvMetadata for time range discovery
QueryPvMetadataResponse DataDecoder::queryPvMetadata(const QueryPvMetadataRequest& request) const {
    return query_client_->queryPvMetadata(request);
}
